...
def call_bedrock_agent(input_text: str, session_id: str) -> str:
    # TODO: Update to use PowerTools SSM Params for optimization
    AGENT_ALIAS_ID = get_ssm_parameter(
        f"/{ENVIRONMENT}/multi-agent-collab-demo/bedrock-agent-alias-id-full-string"
    )
    AGENT_ALIAS_ID = AGENT_ALIAS_ID.split("|")[-1]
    AGENT_ID = get_ssm_parameter(
        f"/{ENVIRONMENT}/multi-agent-collab-demo/bedrock-agent-id"
    )
